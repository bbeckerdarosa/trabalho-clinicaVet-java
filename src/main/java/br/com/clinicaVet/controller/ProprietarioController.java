...
public class ProprietarioController {
	...
	@DeleteMapping(value = "/proprietario/{cpf}")
	public ResponseEntity<?> deletar(@PathVariable("cpf") String cpf) {
		this.proprietarioService.deleteByCpf(cpf);
		return new ResponseEntity<>(HttpStatus.OK);
	}

	@PutMapping(value = "/proprietario/{cpf}")
	public ResponseEntity<?> update(@RequestBody ProprietarioDTO proprietarioDTO) {
		this.proprietarioService.update(proprietarioDTO);
		return new ResponseEntity<>(HttpStatus.OK);
	}
}
